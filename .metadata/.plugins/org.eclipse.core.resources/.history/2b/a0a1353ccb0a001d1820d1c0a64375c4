...
/* method consultar dynamic jpql
 * var parametros = new HashMap<String, Object>(); var jpql = new
 * StringBuilder();
 * 
 * jpql.append("from Restaurante where 0 = 0 ");
 * 
 * if(StringUtils.hasLength(nome)) { jpql.append("and nome like :nome ");
 * parametros.put("nome", "%" + nome + "%"); } if(taxaFreteInicial != null) {
 * jpql.append("and taxaFrete >= :taxaInicial "); parametros.put("taxaInicial",
 * taxaFreteInicial); } if(taxaFreteFinal != null) {
 * jpql.append("and taxaFrete <= :taxaFinal "); parametros.put("taxaFinal",
 * taxaFreteFinal); } TypedQuery<Restaurante> query =
